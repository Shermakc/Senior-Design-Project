...
INSERT INTO person (ID, FirstName, LastName, MiddleName, HomePhone, CellPhone, AddressID, InsuranceProvider, IsPatient, ContactID, ContactRelationship)
SELECT
    ID,
    FirstName,
    LastName,
    ...
    InsuranceProvider,
    IsPatient,
    CASE WHEN NOT IsPatient THEN FLOOR(1 + RAND() * 1000) ELSE NULL END AS ContactID,
    CASE WHEN NOT IsPatient THEN ELT(FLOOR(1 + RAND() * 8), 'Brother', 'Sister', 'Mother', 'Daughter', 'Father', 'Son', 'Cousin', 'Friend') ELSE NULL END AS ContactRelationship
FROM (
    SELECT
        (seq.id + 1) AS ID,
        ...
        -- Random IsPatient (true or false) stored for reuse
        CASE WHEN RAND() > 0.25 THEN TRUE ELSE FALSE END AS IsPatient
